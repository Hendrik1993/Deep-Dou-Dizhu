...
class Env(object):
    ...
    def run(self, is_training=False):
        ...
        while not self.is_over():
            # Agent plays
            if not is_training:
                action, log_prob, value, entropy = self.agents[player_id].eval_step(state)
            else:
                action, _, _ = self.agents[player_id].step(state)

            # Environment steps
            next_state, next_player_id = self.step(action, self.agents[player_id].use_raw)
            # Save action
            #trajectories[player_id].append(action)
            trajectories[player_id].append([action, log_prob, value, entropy])
